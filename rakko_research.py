...
# ── 調査対象のシード語（編集して使う）─────────────────────────────
SEEDS = [
    # 各ツールの主要語（タイトル/見出し最適化のため上限なしで取得）
    "名前 ジェネレーター", "かっこいい名前", "ギルド名 メーカー",
    "二つ名 ジェネレーター", "技名 ジェネレーター", "必殺技 名前",
    "チーム名 ジェネレーター", "サーバー名 おしゃれ", "プレイヤーネーム",
    "ゲーム id おしゃれ", "フォント 変換", "文字 装飾",
    "ガチャ シミュレーター", "おみくじ 無料", "ルーレット メーカー",
    "あみだくじ", "ガチャ 確率 計算",
]
OUT_TAG = "seo"  # 出力ファイル名のサフィックス
VOL_MIN, VOL_MAX = 0, 1000000   # 上限なし（見出し語＝高ボリューム語も拾う）
