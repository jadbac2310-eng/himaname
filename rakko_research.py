...
# ── 調査対象のシード語（編集して使う）─────────────────────────────
SEEDS = [
    # A. ランダム選択・抽選（暇つぶし＆みんなで使う）
    "ルーレット", "あみだくじ", "チーム分け", "グループ分け", "順番 決める",
    "ランダム 選択", "おみくじ", "ガチャ シミュレーター", "ガチャ 確率",
    # B. ゲーム系計算・ツール
    "ダメージ計算", "経験値 計算", "確率 計算", "サイコロ", "タイマー",
    "ストップウォッチ",
    # C. 文字・装飾系（SNS/ゲーム名に使う）
    "特殊文字", "顔文字", "文字 装飾", "フォント 変換", "ローマ字 変換",
    "文字数 カウント", "名前 ローマ字",
    # D. 暇つぶし・診断ツール
    "暇つぶし ゲーム", "診断メーカー", "タイピング 練習", "早口言葉",
    "パスワード 生成", "クイズ 作成",
]
OUT_TAG = "tools"  # 出力ファイル名のサフィックス
